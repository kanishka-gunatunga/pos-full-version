...
import { Payment, CreatePaymentPayload, IndividualPaymentPayload } from "@/types/payment";
...
export default function ProcessPaymentModal({
  ...
  const orderId = payment.id || payment.orderNo;

  const { data: loyaltyData } = useGetLoyaltyPoints(customerMobile || "");
  const availablePoints = loyaltyData?.loyaltyPoints || 0;
  const hasCustomerForPoints = loyaltyData?.customerId != null;
  ...
  const handleConfirmPayment = async () => {
    ...
    try {
      ...
      const payload: CreatePaymentPayload = {
        ...
        status: "paid",
        payments,
        ...(payment.isAdditionalCharge ? { paymentRole: "balance_due" } : {}),
      };
